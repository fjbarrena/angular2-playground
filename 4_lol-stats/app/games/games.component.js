"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var rest_http_1 = require('../servicios/rest.http');
var GamesComponent = (function () {
    function GamesComponent(restHttp) {
        this.restHttp = restHttp;
        console.log("Entrando en DashboardComponent");
    }
    GamesComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.conexion = "Estoy conectado a Mis Partidas";
        this.restHttp.getMyGames()
            .subscribe(function (games) { return _this.handleSuccess(games); }, function (error) { return _this.errorMessage = error; });
    };
    GamesComponent.prototype.handleSuccess = function (games) {
        this.conexion = JSON.stringify(games);
    };
    GamesComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            selector: 'my-games',
            templateUrl: 'mygames.component.html',
            providers: [rest_http_1.RestHttp]
        }), 
        __metadata('design:paramtypes', [rest_http_1.RestHttp])
    ], GamesComponent);
    return GamesComponent;
}());
exports.GamesComponent = GamesComponent;
//# sourceMappingURL=games.component.js.map